...
class ViewController2: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        txbtn = UIButton(frame: CGRect(x: self.view.bounds.maxX / 2 - 100, y: self.view.bounds.maxY / 2 + 10, width: 200, height: 40))
        txbtn.setTitle("一键退学", for: .normal)
        txbtn.setTitleColor(charColor, for: .normal)
        txbtn.addTarget(for: .touchUpInside, action: {
            (UIButton) -> Void in
            UIApplication.shared.open(URL(string: "tel://021-54745845")!, options: [:], completionHandler: { (success) in
